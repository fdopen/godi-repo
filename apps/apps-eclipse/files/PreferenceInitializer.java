...
public class PreferenceInitializer extends AbstractPreferenceInitializer {
    ...
    {  
		try {
			...
			String fullyQualifiedExecutable = "";  
			for (String pathDir : pathDirs)  
			{  
				File file = new File(pathDir, executableName);  
				if (file.isFile())  
				{  
					fullyQualifiedExecutable = file.getPath();  
					break;  
				}  
			}  
	...
	public void initializeDefaultPreferences() {
		try {
			...
			// on Windows:
				...
				ocaml = findExecutableOnPath("ocaml.exe");
					
				ocamlc = findExecutableOnPath("ocamlc.opt.exe");
				if (ocamlc.equals("")){
					ocamlc = findExecutableOnPath("ocamlc.exe");
				}
				
				ocamlopt = findExecutableOnPath("ocamlopt.opt.exe");
				if (ocamlopt.equals("")){
					ocamlopt = findExecutableOnPath("ocamlopt.exe");
				}

				ocamldep = findExecutableOnPath("ocamldep.opt.exe");
				if (ocamldep.equals("")){
					ocamldep = findExecutableOnPath("ocamldep.exe");
				}

				ocamllex = findExecutableOnPath("ocamllex.opt.exe");
				if (ocamllex.equals("")){
					ocamllex = findExecutableOnPath("ocamllex.exe");
				}

				ocamlyacc = findExecutableOnPath("ocamlyacc.opt.exe");
				if (ocamlyacc.equals("")){
					ocamlyacc = findExecutableOnPath("ocamlyacc.exe");
				}

				ocamldoc = findExecutableOnPath("ocamldoc.opt.exe");
				if (ocamldoc.equals("")){
					ocamldoc = findExecutableOnPath("ocamldoc.exe");
				}

				camlp4 = findExecutableOnPath("camlp4.opt.exe");
				if (camlp4.equals("")){
					camlp4 = findExecutableOnPath("camlp4.exe");
				}

				ocamlbuild = findExecutableOnPath("ocamlbuild.opt.exe");
				if (ocamlbuild.equals("")){
					ocamlbuild = findExecutableOnPath("ocamlbuild.exe");
				}

				ocamldebug = findExecutableOnPath("ocamldebug.exe");
				omake = findExecutableOnPath("omake.exe");
				make = findExecutableOnPath("make.exe");
				
				ocamlLibPath = System.getenv("OCAMLLIB");
				if ( ocamlLibPath == null ){
					ocamlLibPath = System.getenv("CAMLLIB");
					if ( ocamlLibPath == null ){
						if ( ocamlc.equals("")){
							ocamlLibPath = "";
						}
							...
							try {
								CommandRunner commandRunner = new CommandRunner(new String[] { ocamlc, "-where" }, null);
								ocamlLibPath = commandRunner.getStdout().trim();
							} catch (Throwable e) {
								ocamlLibPath = "";
							}
